...
import numpy as np
import imutils
import pickle    #like an json file instead using pickle
import cv2
import os
...
embeddingModel = "openface_nn4.small2.v1.t7" #initializing model for embedding Pytorch(face detection)

#initialization of caffe model for face detection,recoganization(pre-trained model)
prototxt = "model/deploy.prototxt"
model =  "model/res10_300x300_ssd_iter_140000.caffemodel"

#loading caffe model for face detection
#detecting face from Image via Caffe deep learning
#using Syntax-readNetFromCaffe calling prototxt &mode
...
for (i, imagePath) in enumerate(imagePaths):
	...
	if len(detections) > 0:
		...
		confidence = detections[0, 0, i, 2]
                ...
                #checking confidence level with thershold vale
		if confidence > conf:
			#ROI range of interest
			box = detections[0, 0, i, 3:7] * np.array([w, h, w, h])
			(startX, startY, endX, endY) = box.astype("int")
			face = image[startY:endY, startX:endX]  #boundary box
			(fH, fW) = face.shape[:2]
